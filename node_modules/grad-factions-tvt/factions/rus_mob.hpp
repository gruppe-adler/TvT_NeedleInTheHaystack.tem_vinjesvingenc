class rus_mob {
    class AllUnits {
        primaryWeapon = "arifle_AK12_F";
        primaryWeaponMagazine = "30Rnd_762x39_Mag_Tracer_Green_F";
        primaryWeaponMuzzle = "";
        primaryWeaponOptics = "rhs_acc_ekp8_18";
        primaryWeaponPointer = "acc_flashlight";
        primaryWeaponUnderbarrel = "";
        primaryWeaponUnderbarrelMagazine = "";
        secondaryWeapon = "";
        secondaryWeaponMagazine = "";
        handgunWeapon = "";
        uniform = "rds_uniform_Rocker2";
        vest[] = {
            "V_LegStrapBag_black_F",
            "V_LegStrapBag_coyote_F",
            "V_LegStrapBag_olive_F"
        };
        backpack[] = {
            "B_LegStrapBag_olive_F",
            "B_LegStrapBag_coyote_F",
            "B_LegStrapBag_black_F"
        };
        headgear = "H_Booniehat_khk_hs";
        nvgoggles = "";
        radio = "TFAR_fadak";
        binoculars = "Binocular";
        map = "ItemMap";
        gps = "ItemGPS";
        compass = "ItemCompass";
        addItemsToUniform[] = {
            LIST_6("ACE_fieldDressing"),
            LIST_2("ACE_morphine"),
            "ACE_epinephrine",
            "ACE_Flashlight_KSF1",
            "ACE_MapTools",
            "ACE_EntrenchingTool"
        };
        addItemsToBackpack[] = {
            LIST_2("rhs_mag_rdg2_white"),
            LIST_4("rhs_mag_an_m14_th3")
        };
    };

    class Type {
        //rifleman
        class Soldier_F {
            uniform[] = {
                "rds_uniform_Rocker1",
                "rds_uniform_Rocker2",
                "rds_uniform_Rocker3",
                "rds_uniform_Rocker4",
                "rds_uniform_citizen1",
                "rds_uniform_citizen2",
                "rds_uniform_citizen3",
                "rds_uniform_citizen4",
                "rds_uniform_profiteer1",
                "rds_uniform_profiteer2",
                "rds_uniform_profiteer3",
                "rds_uniform_profiteer4",
                "rds_uniform_assistant"
            };
            headgear[] = {
                "H_Hat_Safari_olive_F",
                "H_Hat_Safari_sand_F",
                "H_HeadBandage_bloody_F",
                "H_HeadBandage_clean_F",
                "rds_Woodlander_cap3",
                "rds_Woodlander_cap2",
                "rds_rocker_hair1",
                "rds_rocker_hair3",
                "rhssaf_booniehat_woodland",
                "rhssaf_bandana_md2camo",
                "rhssaf_bandana_digital",
                "rhsgref_bcap_specter",
                "rhs_beanie_green",
                "H_Hat_brown",
                "H_Booniehat_khk_hs",
                "",
                "",
                "",
                ""
            };
            addItemsToVest[] = {
                LIST_7("30Rnd_762x39_Mag_Tracer_Green_F")
            };
        };

        //autorifleman
        class Soldier_AR_F: Soldier_F {
            primaryWeapon = "rhs_weap_pkp";
            primaryWeaponMagazine = "rhs_100Rnd_762x54mmR";
            backpack = "rhssaf_kitbag_smb";
            headgear = "H_Cap_headphones";
            addItemsToVest[] = {
                LIST_1("rhs_100Rnd_762x54mmR")
            };
            addItemsToBackpack[] = {
                LIST_2("rhs_100Rnd_762x54mmR_green")
            };
        };

        //light AT
        class Soldier_LAT_F: Soldier_F {
            secondaryWeapon = "rhs_weap_rpg26";
        };

        //ammo bearer
        class Soldier_A_F: Soldier_F {
            backpack = "rhssaf_kitbag_digital";
            addItemsToBackpack[] = {
                LIST_2("rhs_100Rnd_762x54mmR_green")
            };
        };

        //assistant autorifleman
        class Soldier_AAR_F: Soldier_F {
            backpack = "SMA_BackPack_Green";
            addItemsToBackpack[] = {
                LIST_2("rhs_100Rnd_762x54mmR_green")
            };
        };

        //medic
        class Medic_F: Soldier_F {
            backpack = "rhssaf_kitbag_digital";
            addItemsToBackpack[] = {
                LIST_20("ACE_fieldDressing"),
                LIST_10("ACE_fieldDressing"),
                LIST_15("ACE_morphine"),
                LIST_15("ACE_epinephrine"),
                LIST_2("ACE_bloodIV_250")
            };
        };

        //team leader
        class Soldier_TL_F: Soldier_F {
            primaryWeapon = "arifle_AK12_GL_F";
            primaryWeaponUnderbarrelMagazine = "1Rnd_SmokeGreen_Grenade_shell";
            headgear = "H_Watchcap_blk";
            addItemsToBackpack[] = {
                LIST_3("1Rnd_SmokeGreen_Grenade_shell"),
                LIST_3("1Rnd_SmokePurple_Grenade_shell"),
                LIST_2("1Rnd_SmokeRed_Grenade_shell"),
                LIST_6("ACE_40mm_Flare_white"),
                LIST_6("ACE_40mm_Flare_green"),
                LIST_6("ACE_40mm_Flare_red")
            };
        };

         //squad leader
        class Soldier_SL_F: Soldier_TL_F {
            uniform = "LOP_U_US_Fatigue_04";
            primaryWeaponOptics = "rhs_acc_1p63";
            headgear = "rhs_fieldcap_khk";
        };
    };

    class Rank {
        class LIEUTENANT {
            headgear = "rhs_fieldcap";
        };
    };
};
